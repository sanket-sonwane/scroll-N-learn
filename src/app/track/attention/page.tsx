...
const EXPERIENCE_ID = "ai-engineering";

export default function AttentionLanding() {
  const track = getTrack("ai-engineering");
  const total = track.cards.length;
  const state = useState(() => readProgress(EXPERIENCE_ID))[0];
    ...
    <main className="flex h-full w-full flex-col items-center justify-center gap-10 overflow-y-auto px-6">
      ...
      >
        ...
        <div className="mt-6 flex flex-col gap-2.5">
          <Link
            href={`/feed/${EXPERIENCE_ID}/${finished ? 0 : resume}`}
          ...
          <button
            type="button"
            onClick={() => clearProgress(EXPERIENCE_ID)}
      ...
      >
        ...
        <Link
          ...
          className="text-sm text-faint transition-colors hover:text-muted"
        >
          Switch track · Algorithms Playground →
        </Link>
      </motion.div>
    </main>
  );
